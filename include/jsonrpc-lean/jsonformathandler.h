...
namespace jsonrpc {
    ...
    public:
        ...
        std::unique_ptr<Reader> CreateReader(const std::string&         data,
                                            Reader::GetNamedParamsFunc func) override
        {
            return std::unique_ptr<Reader>(std::make_unique<JsonReader>(data, func));
        }
